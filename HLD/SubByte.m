...
function state = SubBytes(state)
    % SubBytes: Performs the SubBytes operation in AES
    % Input:
    %   state - 4x4 matrix of bytes (current state)
    % Output:
    %   state - 4x4 matrix of bytes (updated state after substitution)
    ...
    for row = 1:4
        for col = 1:4
            ...
            sboxRow = bitshift(byte, -4) + 1; % Upper 4 bits
            sboxCol = bitand(byte, 0x0F) + 1; % Lower 4 bits
